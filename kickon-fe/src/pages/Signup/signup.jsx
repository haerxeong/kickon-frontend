...
const Signup = () => {
    ...
    useEffect(() => {
        ...
        if (selectedLeague) {
            const teams = leagues.find(league => league.name === selectedLeague)?.teams || [];
    ...
    useEffect(() => {
        ...
        const requiredAgreements = isAgeAgreed && isServiceTermsAgreed && isPrivacyPolicyAgreed;
        const isFormValid = nickname.length >= 2 && selectedLeague && selectedTeam && requiredAgreements;
    }, [nickname, selectedLeague, selectedTeam, isAgeAgreed, isServiceTermsAgreed, isPrivacyPolicyAgreed]);
    ...
    return (
        <SignupContainer>
            ...
            <InputGroup>
                ...
                <Dropdown onClick={() => setIsLeagueDropdownOpen(!isLeagueDropdownOpen)}>
                    <DropdownContent>
                        <DropdownText>
            ...
            <CheckboxWrapper onClick={handleAllAgreementToggle}>
                {isAllAgreed ? <FaCheckSquare color="#C00C0B" /> : <FaRegCheckSquare color="#999" />}
                <CheckboxLabel>모두 동의</CheckboxLabel>
            </CheckboxWrapper>

            <CheckboxWrapper
                onClick={() => setIsAgeAgreed(!isAgeAgreed)}
            >
                {isAgeAgreed ? <FaCheckSquare color="#C00C0B" /> : <FaRegCheckSquare color="#999" />}
            ...
            >
                {isServiceTermsAgreed ? <FaCheckSquare color="#C00C0B" /> : <FaRegCheckSquare color="#999" />}
                <CheckboxLabel>
                    서비스 이용약관 동의 (필수)
            ...
            >
                {isPrivacyPolicyAgreed ? <FaCheckSquare color="#C00C0B" /> : <FaRegCheckSquare color="#999" />}
                <CheckboxLabel>
                    개인정보처리방침 동의 (필수)
                    <ViewTermsLink>약관 보기</ViewTermsLink>
                </CheckboxLabel>
            ...
            >
                {isMarketingAgreed ? <FaCheckSquare color="#C00C0B" /> : <FaRegCheckSquare color="#999" />}
                <CheckboxLabel>마케팅 정보 수신 동의 (선택) <ViewTermsLink>약관 보기</ViewTermsLink> </CheckboxLabel>
            </CheckboxWrapper>
